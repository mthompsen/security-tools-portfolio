...
SUSPICIOUS_UA_PATTERNS = [
    'sqlmap', 'nikto', 'nmap', 'masscan', 'wpscan', 'dirbuster', 'gobuster',
    'hydra', 'metasploit', 'burpsuite', 'curl/', 'wget/', 'python-requests',
    'go-http-client', 'libwww-perl',
]

# Paths commonly probed by automated scanners
SUSPICIOUS_PATH_PATTERNS = [
    '/wp-admin', '/wp-login', '/.env', '/.git', '/admin', '/phpmyadmin',
    '/.aws/credentials', '/etc/passwd', '/web.config', '/server-status',
    '/wp-config.php', '/.ssh/', '/api/v1/auth', '/.htaccess',
]
...
def report(result: AnalysisResult, threshold: int, show_all: bool) -> None:
    ...
    # Parse statistics
    parse_rate = (
        (result.parsed_lines / result.total_lines * 100)
        if result.total_lines else 0
    )
...
def main() -> int:
    ...
    if fmt == 'auto':
        ...
        if fmt == 'unknown':
            print(
                'error: could not auto-detect log format. '
                'Specify with --format.',
                file=sys.stderr,
            )
            return 2
