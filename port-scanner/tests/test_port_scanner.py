...
class TestResolveTarget:
    ...
    def test_rejects_invalid_hostname(self):
        ...
        with pytest.raises(ValueError):
            port_scanner.resolve_target(
                'this-host-does-not-exist-at-all.invalid'
            )
...
class TestScanPort:
    ...
    async def test_open_port_detected(self, echo_server):
        ...
        result = await port_scanner.scan_port(
            '127.0.0.1', port, timeout=2.0, sem=sem, banner_grab=False,
        )
    ...
    async def test_closed_port_detected(self):
        ...
        sem = asyncio.Semaphore(1)
        result = await port_scanner.scan_port(
            '127.0.0.1', 1, timeout=2.0, sem=sem, banner_grab=False,
        )
    ...
    async def test_timeout_classified_as_filtered(self, monkeypatch):
        ...
        sem = asyncio.Semaphore(1)
        result = await port_scanner.scan_port(
            '127.0.0.1', 9999, timeout=0.05, sem=sem, banner_grab=False,
        )
